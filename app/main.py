...
class ImageRequest(BaseModel):
    ...
    output_width: Optional[int] = None  # Optionale Ausgabegröße
    output_height: Optional[int] = None  # Optionale Ausgabegröße
...
async def process_image(
    ...
    try:
        ...
        # Test: Hardcodierter Text zum Debuggen
        draw = ImageDraw.Draw(background_image)
        test_font_path = "app/fonts/arial.ttf"  # Stelle sicher, dass dieser Pfad korrekt ist
        test_font_size = 50
        test_font = ImageFont.truetype(test_font_path, test_font_size)
        test_text = "Testtext: Funktioniert es?"
        test_position = (50, 50)  # X, Y-Position
        draw.text(test_position, test_text, font=test_font, fill="red")
        logging.debug(f"Testtext '{test_text}' wurde gezeichnet.")
